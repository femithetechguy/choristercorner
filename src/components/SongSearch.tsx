...
export default function SongSearch({ onSelect, onClose }: SongSearchProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4 pointer-events-auto">
      <div className="bg-gray-900 rounded-lg shadow-2xl border-2 border-purple-500 pointer-events-auto w-full max-w-2xl max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className="p-4 bg-gray-800 border-b border-gray-700 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-2">
            <Search className="w-5 h-5 text-gray-400" />
            <h3 className="text-white font-semibold">Find Next Song</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-4 bg-gray-800 border-b border-gray-700 flex-shrink-0 space-y-3">
          ...
          {/* Channel Filter and Count - Responsive Grid */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 items-end">
            <select
              value={selectedChannel}
              onChange={(e) => setSelectedChannel(e.target.value)}
              className="bg-gray-700 text-white rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-purple-500"
            >
            ...
            <div className="space-y-2 p-4">
              {results.map((song) => (
                <button
                  ...
                  className="w-full flex gap-3 p-3 bg-gray-800 hover:bg-purple-600 rounded transition-colors text-left"
                >
